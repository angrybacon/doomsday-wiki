...
- A colon right in front of a word will be interpreted as
  *directive* so make sure you have a space right after =:= when it used as
  punctuation

** Scryfall

In some contexts, you can refer to card names within Markdown; various
representations are available. To ease the writing process, a number of
...
https://scryfall.com/sets . If not provided, the default set used is the first
print for the card.
...
*Centered* rows with more than 4 cards will start shrinking the images so that
